<?php 
	...
	if(isset($_POST['submit']) && $_POST['submit']=='Submit') {
		...
		$sql = "insert into locationQuestions (parentId,question,answer,publish,createdOn,updatedOn)values('{$parentId}','{$question}','{$answer}','{$publish}','{$create_time}','{$create_time}')";
		$query = mysqli_query($con,$sql);
		$lastInsertId = mysqli_insert_id($con);
		
		if($lastInsertId) {
			$_SESSION['msg'] = 'data_uploaded';
			header("location: view-location-question.php?parentId=".$parentId1);
		} 
		else{
			header("location: view-location-question.php?parentId=".$parentId1);exit;
		}
	} 
    ...
    if(isset($_POST['submit']) && $_POST['submit']=='Save Changes') {
		...
		$sql = "update locationQuestions set parentId = '{$parentId}',question = '{$question}',answer = '{$answer}',updatedOn = '{$create_time}' where id = '$id'";
	...
	if($_SERVER["REQUEST_METHOD"]== "GET" && isset($_GET['action']) && $_GET['action']=='status'){
		...
		$sql = "select publish from locationQuestions where id = {$id} ";
		$query = mysqli_query($con,$sql);
		...
		$sql = "update locationQuestions set publish = {$status} where id = {$id}";
	...
	if($_SERVER["REQUEST_METHOD"]== "GET" && isset($_GET['action']) && $_GET['action']=='delete'){
		...
		$sql = "delete from locationQuestions where id = {$id}";
