...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>User List
			<a href="add-user.php" class="btn btn-primary pull-right" style="margin-top:-5px;"><i class="fa fa-plus"></i>&nbsp;Add User</a>
		</h1>
	</section>
	<!-- Main content -->
				...
				<div class="box">
	            	...
	            	<?php if(isset($_SESSION['msg']) && $_SESSION['msg']=='delete_data') { ?>
					  ...
					  <table id="employee-role-table" class="table table-bordered table-striped">
						...
						<tbody>
						<?php
							$fetch_blog=mysqli_query($con,"SELECT users.* FROM `users` where is_deleted = 0 ORDER BY users.id DESC") or die(mysqli_error($con));
							$i=1;
						...
						?>
						  <tr>
							...
							<td><?php echo $i;?></td>
							<td><a href="edit-user.php?id=<?php echo base64_encode($data['id']); ?>"><?php echo $data['fname'];?></a></td>
							...
							<td>
								<?php 
									if( $data['image']!="")
									{
										$imgfileSource = ".." . DIRECTORY_SEPARATOR . 'public/uploads/users_img' . DIRECTORY_SEPARATOR . $data['image'];
									}	
									else{
										$imgfileSource = ".." . DIRECTORY_SEPARATOR . 'images/' . DIRECTORY_SEPARATOR . 'dummy-profile.jpg';
									}	
						...
						<button type="submit" class="btn btn-primary btn_fnt" name="btn_publish" id="btn_publish"><i class="icon dripicons-trash color_change"></i>Active</button>
						<button type="submit" class="btn btn-warning btn_fnt" name="btn_unpublish" id="btn_unpublish"><i class="icon dripicons-trash color_change"></i>Deactive</button>
					</div><!-- /.box-body -->
				  </div><!-- /.box -->
			</div>
		</div>
...
$(".statusButton").click(function() {
	...
	if(confirm("Do you really want to change the status of this record?")) {
		window.location = "user-process.php?action=status&id="+dataid;
	}
});
...
$(".deleteButton").click(function() {
	...
	if(confirm("Do you really want to delete this record?")) {
		window.location = "user-process.php?action=delete&id="+dataid;
	}
});
