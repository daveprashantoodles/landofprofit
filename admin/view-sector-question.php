...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>Sector Question List
			<a href="add-sector-question.php?parentId=<?php echo $parentId;?>" class="btn btn-primary pull-right" style="margin-top:-5px;"><i class="fa fa-plus"></i>&nbsp;Add Sector Question</a>
		</h1>
	</section>
					...
					<div class="box-body">
						...
						<table id="employee-role-table" class="table table-bordered table-striped">
							...
							// echo "SELECT * FROM `serviceQuestions` where parentId = '{$id}'  ORDER BY `id` DESC";exit;
								$fetch_blog=mysqli_query($con,"SELECT * FROM `sectorQuestions` where parentId = '{$id}'  ORDER BY `id` DESC") or die(mysqli_error($con));
								$i=1;
							...
							?>
									<tr>
										...
										<td><?php echo $i;?></td>
										<td>
											<a href="edit-sector-question.php?parentId=<?php echo $parentId;?>&id=<?php echo base64_encode($data['id']); ?>">
...
$(".statusButton").click(function() {
	...
	if(confirm("Do you really want to change the status of this record?")) {
		window.location = "sector-question-process.php?action=status&parentId="+parentId+"&id="+dataid;
	}
});
...
$(".deleteButton").click(function() {
	...
	if(confirm("Do you really want to delete this record?")) {
		window.location = "sector-question-process.php?action=delete&parentId="+parentId+"&id="+dataid;
	}
});
...
$('button[name=btn_delete]').click(function ()
	...
	else
	{
	  	if (confirm("Are you sure you want to delete records on this page?"))
	  	{
	  		$.ajax(
	  		{
	  			url: "deleteSectorQuestion.php?parentId="+parentId,
	  			method: 'POST',
				dataType :"json",
	  			data:
	  			{
...
$('button[name=btn_publish]').click(function ()
	...
	else
	{
	  	if (confirm("Do you want to publish these records?"))
	  	{
	  		$.ajax(
	  		{
	  			url: "publishSectorQuestion.php?parentId="+parentId,
	  			method: 'POST',
				dataType :"json",
	  			data:
	  			{
...
$('button[name=btn_unpublish]').click(function ()
	...
	else
	{
	  	if (confirm("Do you want to unpublish these records?"))
	  	{
	  		$.ajax(
	  		{
	  			url: "unpublishSectorQuestion.php?parentId="+parentId,
	  			method: 'POST',
				dataType :"json",
	  			data:
	  			{
