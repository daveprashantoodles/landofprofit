...
$(document).ready(function() {
	$("#albumform").validate({
		...
		albumSlug: {
				required: true,
				remote: "checkDuplicateAlbum.php",
		},
		},
		messages: {
			albumName: {
			...
			albumSlug: {
				required: "This field cannot be blank.",
				remote: "Album Slug already exist.",
			},
...
<section class="content-header">
    <h1>Add Photo<!--<small>Preview</small>--></h1>
</section>
				...
				<div class="box box-info">
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                     <label for="name" class="col-lg-4 control-label">Image Name <span class="mandatory">*</span></label>
                     <div class="col-lg-8">
                      <input type="text" class="form-control isRequired" id="albumName" name="albumName" placeholder="Enter Name" title="Please enter name" />
                     </div>
                    </div>
                  </div>
				...
				<div class="box">
	            	...
	            	<?php if(isset($_SESSION['msg']) && $_SESSION['msg']=='delete_data') { ?>
					  ...
					  <table id="employee-role-table" class="table table-bordered table-striped">
						...
						<tbody>
						<?php
							$fetch_blog=mysqli_query($con,"SELECT * FROM `ifd_album` ORDER BY `album_id` DESC")or die(mysqli_error($con));
							$i=1;
						...
						?>
						  <tr>
							...
							<?php $albumImg='school-photo-gallery';?>
							...
							// $actual_link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
								...
								$imgfileSource = $base_url. DIRECTORY_SEPARATOR . $albumImg . DIRECTORY_SEPARATOR . $data['image'];
								//if(file_exists($imgfileSource)){ 
									echo  $imgfileSource; 
								//} 
							 ?>
							</td>
							<!--<td><a title="Click here to View Galley" href="view-gallery.php?id=<?php //echo base64_encode($data['album_id']);?>"><span class="btn btn-primary"><i class="fa fa-image"> View Gallery</span></a></td>-->
							
							<td>
							  <a href="edit-school-photo.php?id=<?php echo base64_encode($data['album_id']); ?>" class="btn btn-default" style="margin-right:2px;margin-bottom:6px;" title="Edit"><i class="fa fa-pencil"> Edit</i></a>
							</td>
							<!--
							<td>
							    
							<?php if($data['status']=="1"){?>
							<a title="Click here to Disable Record" class="show-pointer"><span class="label label-success statusButton" id="<?php echo base64_encode($data['album_id']);?>">Active</span></a>	
						<?php	}
						 if($data['status']=="0"){?>
							<a title="Click here to Active This Record" class="show-pointer"><span class="label label-warning statusButton" id="<?php echo base64_encode($data['album_id']);?>">Deactive</span></a>	
						<?php	}?>
						   </td>
						   -->
...
$(".statusButton").click(function() {
	...
	if(confirm("Do you really want to change the status of this record?")) {
		window.location = "album-process.php?action=status&id="+dataid;
	}
});
...
$(".deleteButton").click(function() {
	...
	if(confirm("Do you really want to delete this record?")) {
		window.location = "album-process.php?action=delete&id="+dataid;
	}
});
