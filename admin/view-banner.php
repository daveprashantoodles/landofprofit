...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>Banner List
			<a href="add-banner.php" class="btn btn-primary pull-right" style="margin-top:-5px;"><i class="fa fa-plus"></i>&nbsp;Add Banner</a>
		</h1>
	</section>
	<!-- Main content -->
				...
				<div class="box">
					<div class="box-body">
						...
						<table id="employee-role-table" class="table table-bordered table-striped">
							...
							<tbody>
							<?php
								$fetch_blog=mysqli_query($con,"SELECT * FROM `banner` ORDER BY `id` DESC") or die(mysqli_error($con));
								$i=1;
							...
							?>
									<tr>
										...
										<td><?php echo $i;?></td>
										<td>
											<a href="edit-banner.php?id=<?php echo base64_encode($data['id']); ?>">
												<?php echo $data['text1'];?>
											</a>
										</td>
										<td>
											<?php echo isset($data['text2'])?$data['text2']:"";?>
										</td>
										<td>
											<?php echo isset($data['text3'])?$data['text3']:"";?>
										</td>
										<td>
										<?php 
...
$(".statusButton").click(function() {
	...
	if(confirm("Do you really want to change the status of this record?")) {
		window.location = "banner-process.php?action=status&id="+dataid;
	}
});
$(".deleteButton").click(function() {
	var dataid = $(this).attr('id');
	if(confirm("Do you really want to delete this record?")) {
		window.location = "banner-process.php?action=delete&id="+dataid;
	}
});
$(function () {
