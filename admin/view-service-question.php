...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>Service Question List
			<a href="add-service-question.php?parentId=<?php echo $parentId;?>" class="btn btn-primary pull-right" style="margin-top:-5px;"><i class="fa fa-plus"></i>&nbsp;Add Service Question</a>
		</h1>
	</section>
					...
					<div class="box-body">
						...
						<table id="employee-role-table" class="table table-bordered table-striped">
							...
							// echo "SELECT * FROM `serviceQuestions` where parentId = '{$id}'  ORDER BY `id` DESC";exit;
								$fetch_blog=mysqli_query($con,"SELECT * FROM `serviceQuestions` where parentId = '{$id}'  ORDER BY `id` DESC") or die(mysqli_error($con));
								$i=1;
							...
							?>
									<tr>
										...
										<td><?php echo $i;?></td>
										<td>
											<a href="edit-service-question.php?parentId=<?php echo $parentId;?>&id=<?php echo base64_encode($data['id']); ?>">
...
$(".statusButton").click(function() {
	...
	if(confirm("Do you really want to change the status of this record?")) {
		window.location = "service-question-process.php?action=status&parentId="+parentId+"&id="+dataid;
	}
});
...
$(".deleteButton").click(function() {
	...
	if(confirm("Do you really want to delete this record?")) {
		window.location = "service-question-process.php?action=delete&parentId="+parentId+"&id="+dataid;
	}
});
...
$('button[name=btn_delete]').click(function ()
	...
	else
	{
	  	if (confirm("Are you sure you want to delete records on this page?"))
	  	{
	  		$.ajax(
	  		{
	  			url: "deleteServiceQuestion.php?parentId="+parentId,
	  			method: 'POST',
				dataType :"json",
	  			data:
	  			{
...
$('button[name=btn_publish]').click(function ()
	...
	else
	{
	  	if (confirm("Do you want to publish these records?"))
	  	{
	  		$.ajax(
	  		{
	  			url: "publishServiceQuestion.php?parentId="+parentId,
	  			method: 'POST',
				dataType :"json",
	  			data:
	  			{
...
$('button[name=btn_unpublish]').click(function ()
	...
	else
	{
	  	if (confirm("Do you want to unpublish these records?"))
	  	{
	  		$.ajax(
	  		{
	  			url: "unpublishServiceQuestion.php?parentId="+parentId,
	  			method: 'POST',
				dataType :"json",
	  			data:
	  			{
