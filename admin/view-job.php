...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>Job List
			<a href="add-job.php" class="btn btn-primary pull-right" style="margin-top:-5px;"><i class="fa fa-plus"></i>&nbsp;Add Job</a>
		</h1>
	</section>
				...
				<div class="box">
					<div class="box-body">
						...
						<table id="employee-role-table" class="table table-bordered table-striped">
							...
							<tbody>
							<?php
								$fetch_blog=mysqli_query($con,"SELECT * FROM `job` ORDER BY `id` DESC") or die(mysqli_error($con));
								$i=1;
							...
							?>
									<tr>
										...
										<td><?php echo $i;?></td>
										<td>
											<a href="edit-job.php?id=<?php echo base64_encode($data['id']); ?>">
										...
										<td>
											<?php echo isset($data['positions'])?$data['positions']:"";?>
										</td>
										<td>
											<?php echo isset($data['last_date'])?$data['last_date']:"";?>
										</td>
										<td>
											<?php echo isset($data['packages'])?$data['packages']:"";?>
										</td>
										<td>
...
$(".statusButton").click(function() {
	...
	if(confirm("Do you really want to change the status of this record?")) {
		window.location = "job-process.php?action=status&id="+dataid;
	}
});
...
$(".deleteButton").click(function() {
	...
	if(confirm("Do you really want to delete this record?")) {
		window.location = "job-process.php?action=delete&id="+dataid;
	}
});
