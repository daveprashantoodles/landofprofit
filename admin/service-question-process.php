<?php 
	...
	if(isset($_POST['submit']) && $_POST['submit']=='Submit') {
		...
		$sql = "insert into serviceQuestions (parentId,question,answer,publish,createdOn,updatedOn)values('{$parentId}','{$question}','{$answer}','{$publish}','{$create_time}','{$create_time}')";
		$query = mysqli_query($con,$sql);
		$lastInsertId = mysqli_insert_id($con);
		
		if($lastInsertId) {
			$_SESSION['msg'] = 'data_uploaded';
			header("location: view-service-question.php?parentId=".$parentId1);
		} 
		else{
			header("location: view-service-question.php?parentId=".$parentId1);exit;
		}
	} 
    ...
    if(isset($_POST['submit']) && $_POST['submit']=='Save Changes') {
		...
		$sql = "update serviceQuestions set parentId = '{$parentId}',question = '{$question}',answer = '{$answer}',updatedOn = '{$create_time}' where id = '$id'";
	...
	if($_SERVER["REQUEST_METHOD"]== "GET" && isset($_GET['action']) && $_GET['action']=='status'){
		...
		$sql = "select publish from serviceQuestions where id = {$id} ";
		$query = mysqli_query($con,$sql);
		...
		$sql = "update serviceQuestions set publish = {$status} where id = {$id}";
	...
	if($_SERVER["REQUEST_METHOD"]== "GET" && isset($_GET['action']) && $_GET['action']=='delete'){
		...
		$sql = "delete from serviceQuestions where id = {$id}";
